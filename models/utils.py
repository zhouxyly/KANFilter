...
def point_cloud_denoising_advanced(points, alpha, beta, gamma, learning_rate, iterations):
    points_tensor = torch.tensor(points, requires_grad=True)
    optimizer = torch.optim.Adam([points_tensor], lr=learning_rate)
    
    for t in range(iterations):
        optimizer.zero_grad()
        
        distances, indices = neighbors_model.kneighbors(points_tensor.detach().numpy())
        densities = 1 / (distances.mean(axis=1) + 1e-6)
        avg_density = np.mean(densities)
        
        surface_loss = compute_surface_loss(points_tensor)
        smoothness_loss = compute_smoothness_loss(points_tensor, indices)
        density_loss = compute_density_loss(points_tensor, densities, avg_density)
        
        total_loss = alpha * surface_loss + beta * smoothness_loss + gamma * density_loss
        total_loss.backward()
        
        optimizer.step()
        
        # 动态调整权重
        alpha, beta, gamma = adjust_weights(surface_loss.item(), smoothness_loss.item(), density_loss.item(), alpha, beta, gamma)
    
    return points_tensor.detach().numpy()

def compute_smoothness_loss(points, neighbors):
    loss = 0
    for i in range(len(points)):
        for j in neighbors[i]:
            loss += np.linalg.norm(points[i] - points[j]) ** 2
    return loss / len(points)

def compute_density_loss(points, densities, avg_density):
    loss = np.sum((densities - avg_density) ** 2)
    return loss / len(points)
