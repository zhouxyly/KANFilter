...
import numpy as np
from .KANModel import KANModule
...
class KANFilter(pl.LightningModule):
    ...
    def __init__(self, args):
        ...
        for i in range(self.num_modules):
            self.feature_nets.append(KANModule(k=self.frame_knn, input_dim=input_dim, z_dim=z_dim, embedding_dim=256, output_dim=3))
            input_dim = 3 + z_dim
    ...
    def training_step(self, train_batch, batch_idx):
        ...
        loss = 0.5 * self.get_supervised_loss_nn(pcl_noisy=pcl_noisy, pcl_clean=pcl_clean, pcl_seeds=pcl_seeds, pcl_std=pcl_std) + 0.5 * self.get_supervised_loss_nn_sp(pcl_noisy=pcl_noisy, pcl_clean=pcl_clean, pcl_seeds=pcl_seeds, pcl_std=pcl_std)    
    ...
    def get_supervised_loss_nn_sp(self, pcl_noisy, pcl_clean, pcl_seeds, pcl_std):
        """
        Denoising score matching.
        Args:
            pcl_noisy:  Noisy point clouds, (B, N, 3).
        ...
        while i < num_patches:
            ...
            try:
                if num_modules_to_use is None:
                    patches_denoised_temp, _ = self.denoise_dynamics(curr_patches, num_modules_to_use=self.num_modules)
                else:
                    patches_denoised_temp, _ = self.denoise_dynamics(curr_patches, num_modules_to_use=num_modules_to_use)
    ...
    def denoise_dynamics(self, pcl_noisy, num_modules_to_use):
        """
        Args:
            pcl_noisy:  Noisy point clouds, (B, N, 3).
        """
